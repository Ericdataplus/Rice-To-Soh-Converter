...
- ✅ **67% of the pack's textures convert out of the box** (Link, NPCs, items, the overworld, UI, dungeons — plus the French/German menus): 62% by direct CRC matching + the rest via the bundled `celda_mapping.json` (see below).
...
### 🎁 The full 67% works out of the box (no extra ROMs needed)

Here's a subtlety we solved for you: texture artists dumped their packs on whatever ROM *they* owned — and Djipi, being French, dumped Celda on a **PAL (European) cart**. A chunk of his textures (French/German menus, PAL-specific plates) can never CRC-match an American archive on its own.

The repo ships **`celda_mapping.json`** — a prebuilt filename→asset-path map (pure metadata, no game data) generated by matching the pack against PAL 1.0 + NTSC 1.0 + GC archives. The script **auto-loads it** when it sits next to `rice_to_soh.py`, so a plain US install still gets the full **67%**. Keep the two files together and there's nothing to configure. (Opt out with `--no-map`.)

<details>
<summary><b>🚀 Converting a different pack? Boost its match rate with <code>--base</code></b></summary>

For packs without a shipped mapping, the same trick works manually: let Ship of Harkinian extract an `oot.o2r` from the ROM revision the pack author used (drop that ROM in a spare SoH folder, launch once, grab the generated `oot.o2r`), then feed it in as an extra matching source:

```bash
python rice_to_soh.py "C:\path\to\Ship of Harkinian" --base oot-pal.o2r
```

Matches are keyed by **asset path**, which is identical across game versions — so textures matched via a PAL archive work perfectly in a US install. European artist? Try the PAL cart. The resulting `matched.csv` can then become a shareable mapping for everyone else.
...
| `--map "<file.json>"` | Use a specific prebuilt mapping (`celda_mapping.json` auto-loads if next to the script) |
| `--no-map` | Pure CRC matching only; skip the auto-loaded mapping |
